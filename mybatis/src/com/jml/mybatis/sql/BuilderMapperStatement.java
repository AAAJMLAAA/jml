...
public class BuilderMapperStatement {
	...
	public static <T> void addStatement(T t) throws Exception {
		...
		maps.put(SqlCommandType.SELECT, BuIderSqlSource.createSelectSqlSource(languageDriver, configuration, t));
		maps.put(SqlCommandType.INSERT, BuIderSqlSource.createInsertSqlSource(languageDriver, configuration, t));
		maps.put(SqlCommandType.UPDATE, BuIderSqlSource.createUpdateSqlSource(languageDriver, configuration, t));
		maps.put(SqlCommandType.DELETE, BuIderSqlSource.createDeleteSqlSource(languageDriver, configuration, t));
		for (Map.Entry<SqlCommandType, SqlSource> map : maps.entrySet())
		{
			MappedStatement.Builder statementBuilder = new MappedStatement.Builder(configuration, class1.getName()+"."+map.getKey().name(),
					map.getValue(), map.getKey())
							.databaseId(databaseId).lang(languageDriver).resultSetType(ResultSetType.SCROLL_SENSITIVE)
